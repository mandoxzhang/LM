...
c10::intrusive_ptr<Work> ProcessGroupMCCL::collective(
  ...
  auto& mcclStreams = mcclStreams_[key];
  // std::cout<<"DEBUG AT Collective : Device Key info = " << key << " Streams are = " << mcclStreams <<std::endl;
  // std::cout<<"DEBUG AT Collective : Events = " << mcclEvents_[key] <<std::endl;
  ...
  //   decltype(i) stream_comm_i = (inputs_same_dev ? 0 : i);
  //   auto& mcclStream = mcclStreams[stream_comm_i];
  //   musaStreamSynchronize(mcclStream);
  // }
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::allreduce_impl(
  ...
  int dev_in_group = 0;
  // std::cout<<"Allreduce Impl is calling to collective -->"<<std::endl;
  return collective(
      tensors,
      tensors,
      [&](at::Tensor& input,
        ...
        mcclResult_t status = mcclSuccess;
        auto mcclReduceOp = getMcclReduceOp(
            opts.reduceOp, input, mcclDataType, comm, dev_in_group++);
//         musaStreamSynchronize(stream.stream());

       status = mcclAllReduce(
            input.data_ptr(),
            output.data_ptr(),
            input.numel(),
            mcclDataType,
            ...
            comm,
            stream.stream());
        // std::cout<<"END OF ALL REDUCE, status = "<<status  <<std::endl;
         return status;
      },
      OpType::ALLREDUCE,
      "mccl:all_reduce");
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::allreduce(
  ...
  // std::cout<<"Allreduce call allreduce impl -->"<<std::endl;
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::allreduce_coalesced(
  ...
  std::cout<<"Allreduce coalesced call allreduce impl -->"<<std::endl;
  // @lint-ignore CLANGTIDY
  RECORD_PARAM_COMMS_DATA(
      static_cast<int>(
          this->getSequenceNumberForGroup() + 1), // seq + 1 to match collective
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::broadcast(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      std::vector<int64_t>(), // inSplitSizes
      std::vector<int64_t>()); // outSplitSizes
  // std::cout<<"Broadcast call to collective -->"<<std::endl;
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::_broadcast_oop(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      std::vector<int64_t>(), // inSplitSizes
      std::vector<int64_t>()); // outSplitSizes
  // std::cout<<"broadcast oop call collective -->"<<std::endl;
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::reduce(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      std::vector<int64_t>(), // inSplitSizes
      std::vector<int64_t>()); // outSplitSizes
  // std::cout<<"reduce call collective -->"<<std::endl;
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::_reduce_oop(
  ...
  int dev_in_group{0};
  // std::cout<<"_reduce oop call collective -->"<<std::endl;
  return collective(
      inputTensors,
      outputTensors,
      [&](at::Tensor& input,
...
// std::cout<<"allgather call to collective -->"<<std::endl;
    return collective(
        inputTensors,
        outputFlattened,
        [&](at::Tensor& input,
    ...
    for (const auto i : c10::irange(num_reduces)) {
      ...
      auto broadcastOpts = BroadcastOptions{
          ...
          static_cast<int64_t>(i % num_devices),
          opts.timeout};
      // std::cout<<"allgather call broadcast_oop-->"<<std::endl;
      auto work =
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::reduce_scatter(
  ...
  if (same_size) {
    ...
    RECORD_PARAM_COMMS_DATA(
        ...
        std::vector<int64_t>(), // inSplitSizes
        std::vector<int64_t>()); // outSplitSizes
// std::cout<<"reduce_scatter call collective -->"<<std::endl;
    return collective(
        inputFlattened,
        outputTensors,
        [&](at::Tensor& input,
    ...
    for (const auto i : c10::irange(num_reduces)) {
      ...
      auto reduceOpts = ReduceOptions{
          ...
          static_cast<int64_t>(i % num_devices),
          opts.timeout};
      // std::cout<<"reduce_scatter call _reduce_oop -->"<<std::endl;
...
// std::cout<<"barrier call allreduce -->"<<std::endl;
...
static mcclResult_t all2all_single_equal_split_impl(
    ...
    mcclComm_t comm,
    c10::musa::MUSAStream& stream) {
  // std::cout<<"DEBUG INFO --> all2all single equal split impl -> mcclSend&Recv" <<std::endl;
  int numranks;
...
static mcclResult_t all2all_single_unequal_split_impl(
  ...
  int numranks;
  // std::cout<<"DEBUG IN all2all single unequal split impl --> SendRecv" <<std::endl;
  C10D_MCCL_ASSERT(mcclCommCount(comm, &numranks));
  C10D_MCCL_ASSERT(mcclGroupStart());
  for (const auto r : c10::irange(numranks)) {
    // MCCL uses 0 byte message for synchronization
...
static mcclResult_t all2all_impl(
    ...
    mcclComm_t comm,
    c10::musa::MUSAStream& stream) {
  // std::cout<<"DEBUG IN all2all impl --> Send&Recv"<<std::endl;
  C10D_MCCL_ASSERT(mcclGroupStart());
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::alltoall_base(
  ...
  if (outputSplitSizes.size() == 0 && inputSplitSizes.size() == 0) {
    ...
    RECORD_PARAM_COMMS_DATA(
        ...
        std::vector<int64_t>(), // inSplitSizes
        std::vector<int64_t>()); // outSplitSizes
  // std::cout<<"all2all base call collective -->"<<std::endl;
    return collective(
        inputTensors,
        outputTensors,
        [&](at::Tensor& input,
  ...
  // std::cout<<"all2all base call collective 2 -->"<<std::endl;
    return collective(
        inputTensors,
        outputTensors,
        [&](at::Tensor& input,
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::alltoall(
  ...
  // std::cout<<"all2all (not base) call collective -->"<<std::endl;
  return collective(
      inputTensor0,
      outputTensor0,
      [&](at::Tensor& /* unused */,
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::send(
    ...
    int /* unused */) {
  check_gpu_tensors_different_devices(tensors);
  std::cout<<"send call p2p -->"<<std::endl;
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::recv(
    ...
    int /* unused */) {
  check_gpu_tensors_different_devices(tensors);
    // std::cout<<"recv call p2p -->"<<std::endl;
  auto ret = pointToPoint(
      tensors,
      [&](at::Tensor& output,
          mcclComm_t comm,
...
mcclResult_t gather_impl(
  ...
  C10D_MCCL_ASSERT(mcclGroupStart());
  // std::cout<<"gather impl call send & recv collective -->"<<std::endl;
  if (cur_rank == root) {
    for (const auto r : c10::irange(numranks)) {
      if (r != root) {
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::gather(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      std::vector<int64_t>(), // inSplitSizes
      std::vector<int64_t>()); // outSplitSize
// std::cout<<"gather call collective to gather impl-->"<<std::endl;
  return collective(
      inputTensors,
      outputs,
      [&](at::Tensor& /* unused */,
...
mcclResult_t scatter_impl(
  ...
  // TODO(yueran-tang): Maybe use autoMcclGroup instead.
  C10D_MCCL_ASSERT(mcclGroupStart());
  // std::cout<<"scatter impl call --> Send&Recv"<<std::endl;
  if (cur_rank == root) {
    for (const auto r : c10::irange(numranks)) {
      if (r != root) {
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::scatter(
  ...
  RECORD_PARAM_COMMS_DATA(
      ...
      std::vector<int64_t>(), // inSplitSizes
      std::vector<int64_t>()); // outSplitSize
  // std::cout<<"scatter call scatter impl to collective -->"<<std::endl;
  return collective(
      outputTensors,
      inputs,
      [&](at::Tensor& /* unused */,
...
c10::intrusive_ptr<Work> ProcessGroupMCCL::_allgather_base(
  ...
  // std::cout<<"_allgather_base call collective-->"<<std::endl;
  return collective(
      inputs,
      outputs,
      [&](at::Tensor& input,

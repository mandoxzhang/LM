...
# from musa_chatglm.modeling_chatglm.modeling_chatglm_for_musa import ChatGLMForConditionalGeneration, ChatGLMConfig
# from musa_chatglm.modeling_chatglm.tokenization_chatglm import ChatGLMTokenizer
# from musa_chatglm.modeling_chatglm.configuration_chatglm import ChatGLMConfig
from chatglm2.modeling_chatglm2_for_musa import ChatGLMForConditionalGeneration
from chatglm2.tokenization_chatglm import ChatGLMTokenizer
        ...
        for pn, param in module.named_parameters(recurse=False):
            ...
            if 'mlp.dense_h_to_4h' in mn:
                if 'weight' in pn:
                    ...
                    param.set_dist_spec(ReplicaSpec())
            elif 'mlp.dense_4h_to_h' in mn:
                if 'weight' in pn:
                    split_param_col_tp1d(param, pg)    # row slice
                    param.compute_spec.set_output_replicate(False)
                    # param.set_dist_spec(ReplicaSpec())
                else:
                    param.set_dist_spec(ReplicaSpec())
            elif 'word_embeddings' in mn or 'wpe' in mn:
                split_param_col_tp1d(param, pg)    # column slice
            elif 'query_key_value' in mn or 'attention.dense' in mn:
                split_param_row_tp1d(param, pg)    # column slice
            else:
                param.set_dist_spec(ReplicaSpec())
            param.visited = True
...
def main():
    ...
    #     )
    config = ChatGLMConfig.from_json_file('./chatglm2/config.json')
    print(config)
    # print(args.model_name_or_path + '/config.json')
    tokenizer = ChatGLMTokenizer.from_pretrained('./chatglm2')
    ...
    logger.info(f'tp init has cost {(time.time() - start) / 60:.3f} mins', [i for i in range(world_size)])
    ...
    logger.info(f'geminiddp has cost {(time.time() - start) / 60} mins', [i for i in range(world_size)])
    checkpoint_io = GeminiCheckpointIO()
    # checkpoint_io.save_sharded_model(model, args.ckpt_path)
    # checkpoint_io.save_sharded_optimizer(model, args.ckpt_path)
    # checkpoint_io.load_sharded_model(model, args.ckpt_path + 'pytorch_model.bin.index.json')
    ...
    # dp_pg = tp_pg.dp_process_group()
    dp_pg_size = dist.get_world_size()
    dp_pg_rank = dist.get_rank()
    train_dataloader = DataLoader(
        train_dataset, sampler=DistributedSampler(train_dataset, num_replicas=dp_pg_size, rank=dp_pg_rank), shuffle=False, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size
    )
    eval_dataloader = DataLoader(
        eval_dataset, sampler=DistributedSampler(eval_dataset, num_replicas=dp_pg_size, rank=dp_pg_rank), collate_fn=default_data_collator, batch_size=args.per_device_eval_batch_size
    )
    test_dataloader = DataLoader(
        test_dataset, sampler=DistributedSampler(test_dataset, num_replicas=dp_pg_size, rank=dp_pg_rank), collate_fn=default_data_collator, batch_size=args.per_device_eval_batch_size
    )

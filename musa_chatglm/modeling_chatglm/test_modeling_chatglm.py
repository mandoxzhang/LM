...
def get_model_and_tokenizer():
    model = AutoModel.from_pretrained(
        "/mnt/vepfs/workspace/zxdu/chatglm_6b", trust_remote_code=True
    ).half()
...
class ChatGLMGenerationTest(unittest.TestCase):
    ...
    def test_stream_chat(self):
        ...
        for prompt, expected_response in zip(prompts, expected_responses):
            response = ""
            for idx, (response, history) in enumerate(
                model.stream_chat(tokenizer, prompt, history=history)
            ):
                pass
            print(repr(response))
            self.assertEquals(expected_response, response)
    ...
    def test_generation(self):
        ...
        for (do_sample, max_length, num_beams), expected_output_sentence in zip(
            parameters, expected_out_sentences
        ):
            set_random_seed(42)
    ...
    def test_batch_generation(self):
        ...
        for (do_sample, max_length, num_beams), expected_output_sentence in zip(
            ...
            batch_out_sentence = tokenizer.batch_decode(

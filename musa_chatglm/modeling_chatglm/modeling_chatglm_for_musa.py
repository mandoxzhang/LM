...
sys.path.append('/home/mccxadmin/haoran/mccl_example')
...
def get_use_rate(m,n,k,t):
    # print(m,n,k)
    return (2*m*n + n*k + 2*m*k)*4/t / 448/1024/1024/1024 , 2*m*n*k*2/t / 15.2/1024/1024/1024/1024
...
class InvalidScoreLogitsProcessor(LogitsProcessor):
    def __call__(
        ...
        # For Musa Compatible
        cpu_scores = scores.detach().clone().to("cpu")
        if torch.isnan(cpu_scores).any() or torch.isinf(cpu_scores).any():
            scores.zero_()
            scores[..., 5] = 5e4
        return scores
...
class SelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        timer.stop('attention_fn', keep_in_history=True)
        elapsed_time = timer.get_timer('attention_fn').get_elapsed_time()
        # print(f'attention_fn has cost {elapsed_time:.3f}')

        timer.start('context_layer')
        output = self.dense(context_layer)
        timer.stop('context_layer', keep_in_history=True)
        elapsed_time = timer.get_timer('context_layer').get_elapsed_time()
        # print(f'context_layer has cost {elapsed_time:.3f}')
        # print(get_use_rate(context_layer.shape[0], context_layer.shape[-1], output.shape[-1], elapsed_time))
...
class GLMBlock(torch.nn.Module):
    ...
    def forward(
        ...
        timer.stop('mlp', keep_in_history=True)
        elapsed_time = timer.get_timer('mlp').get_elapsed_time()
        # print(f'mlp has cost {elapsed_time:.3f}')
...
class ChatGLMPreTrainedModel(PreTrainedModel):
    ...
    def get_masks(self, input_ids, device):
        ...
        # For Musa Compatible
        if "musa" in str(device):
            attention_mask = torch.ones(
                (batch_size, seq_length, seq_length), device="cpu"
            )
            # attention_mask = torch.ones((seq_length, seq_length), device=device)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def __init__(self, config: ChatGLMConfig, empty_init=True):
        ...
        #self.word_embeddings = self.word_embeddings.to('musa')
...
class ChatGLMForConditionalGeneration(ChatGLMPreTrainedModel):
    ...
    def forward(
        ...
        timer.stop('lm_head', keep_in_history=True)
        elapsed_time = timer.get_timer('lm_head').get_elapsed_time()
        # print(f'lm_head has cost {elapsed_time:.3f}')
        # print(get_use_rate(hidden_states.shape[0], hidden_states.shape[-1], lm_logits.shape[-1], elapsed_time))

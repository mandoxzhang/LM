...
class GenerationMixin:
    ...
    def _prepare_attention_mask_for_generation(
        ...
        is_pad_token_in_inputs = (pad_token_id is not None) and (
            pad_token_id in inputs.clone().to("cpu")
        )
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        if input_ids is not None:
            ...
            # input_ids = input_ids.repeat_interleave(expand_size, dim=0) 4.25.1
        ...
        if model_kwargs.get("token_type_ids") is not None:
            ...
            #     "token_type_ids"
            # ].repeat_interleave(expand_size, dim=0) 4.25.1

        if model_kwargs.get("attention_mask") is not None:
            token_type_ids = model_kwargs["attention_mask"]
            model_kwargs["attention_mask"] = token_type_ids.index_select(
                0, expanded_return_idx
            )
    ...
    def contrastive_search(
        ...
        **model_kwargs,
        ...
        # Compatible For Musa
        ori_device = input_ids.device
        if str(ori_device).startswith("musa"):
            input_ids = input_ids.to("cpu")
            unfinished_sequences = input_ids.new(input_ids.shape[0]).fill_(1)
            unfinished_sequences = unfinished_sequences.to(ori_device)
        else:
            input_ids = input_ids.to(ori_device)
            unfinished_sequences = input_ids.new(input_ids.shape[0]).fill_(1)
        ...
        while True:
            ...
            if model_kwargs.get("past") is None:
                ...
                if past is None:
                    ...
                    not isinstance(past[0], (tuple, torch.Tensor))
                    or past[0][0].shape[0] != batch_size
                ):
                    raise ValueError(
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            ori_device = probs.device
            if str(ori_device).startswith("musa"):
                probs = probs.to("cpu")
                next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
                next_tokens = next_tokens.to(ori_device)
                probs = probs.to(ori_device)
            else:
                next_tokens = torch.multinomial(probs, num_samples=1).squeeze(1)
            # finished sentences should have their next token be a padding token
            if eos_token_id is not None:
                ...
                next_tokens = next_tokens  # 转回long

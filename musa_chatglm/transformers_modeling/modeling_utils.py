...
def get_parameter_device(
    parameter: Union[nn.Module, "ModuleUtilsMixin"]
):
    try:
...
def load_sharded_checkpoint(model, folder, strict=True):
    ...
    if not os.path.isfile(index_file):
        raise ValueError(
            f"Can't find a checkpoint index ({WEIGHTS_INDEX_NAME}) in {folder}."
        )
...
def find_submodule_and_param_name(model, long_key, start_prefix):
    """
    A helper utils to find the last sub-module and the param/buffer name. If `start_prefix` is supplied it'll be removed
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, PushToHubMixin):
    ...
    def _init_weights(self, module):
        ...
        """
        raise NotImplementedError(
            f"Make sure `_init_weights` is implemented for {self.__class__}"
        )
        ...
        * `low_cpu_mem_usage` algorithm:
        ...
        if isinstance(device_map, str):
            ...
            if device_map not in ["auto", "balanced", "balanced_low_0", "sequential"]:
                ...
                device_map in ["balanced", "balanced_low_0"]
                and get_balanced_memory is None
            ):
                raise ValueError(
            ...
            if load_in_8bit:
                ...
                device_map_without_lm_head = {
                    ...
                    if key not in modules_to_not_convert
                }
                if (
        ...
        if state_dict is not None:
            ...
            if is_sharded_safetensors:
                disk_only_shard_files = get_disk_only_shard_files(
                    device_map, sharded_metadata=sharded_metadata
                )
    ...
    def _load_pretrained_model_low_mem(
        model, loaded_state_dict_keys, resolved_archive_file, start_prefix=""
    ):
        """
        ...
        error_msgs = _load_state_dict_into_meta_model(
            model, state_dict, loaded_state_dict_keys, start_prefix
        )
        return error_msgs
...
class SQuADHead(nn.Module):
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_log_probs = nn.functional.softmax(
                start_logits, dim=-1
            )  # shape (bsz, slen)
            ...
            end_log_probs = nn.functional.softmax(
                end_logits, dim=1
        ...
        new_device_map.update(
            {
                ...
                if p == module or p.startswith(f"{module}.")
            }
        )
